...
// recupera dati da form in index.html
      ...
      console.log("Dati salvati con successo nel database:", newNote);
      res.redirect("/");
} catch (error) {
        console.error(error);
        res.status(500).send("Errore interno del server");
...
// Definizione dello schema del modello (Nome)
const nomeSchema = new mongoose.Schema({
  nome: String
});

// Creazione del modello Nome
const Nome = mongoose.model('Nome', nomeSchema);

// Middleware per analizzare il corpo della richiesta POST
app.use(bodyParser.urlencoded({ extended: true }));

// Endpoint per gestire la richiesta POST di salvataggio del nome
app.post('/salva-nome', async (req, res) => {
  try {
    const { nome } = req.body;

    // Creazione di un nuovo documento Nome
    const nuovoNome = new Nome({
      nome: nome
    });

    // Salvataggio nel database
    await nuovoNome.save();

    res.send('Nome salvato con successo!');
  } catch (error) {
    console.error(error);
    res.status(500).send('Errore interno del server durante il salvataggio del nome.');
  }
});
